...
CFLAGS:= $(CFLAGS) -DPOWERCAP=1
    ifdef NO_POWER_MANAGEMENT
    CFLAGS:= $(CFLAGS) -DNO_POWER_MANAGEMENT=1
    else
    CFLAGS:= $(CFLAGS) -DNO_POWER_MANAGEMENT=0
    endif
    ifdef POWERCAP_SAMPLE_FILTERING
    CFLAGS:= $(CFLAGS) -DPOWERCAP_SAMPLE_FILTERING=$(POWERCAP_SAMPLE_FILTERING)
    else
    CFLAGS:= $(CFLAGS) -DPOWERCAP_SAMPLE_FILTERING=0
    endif
    ifdef DEBUG_SMALL_SAMPLES
    CFLAGS:= $(CFLAGS) -DDEBUG_SMALL_SAMPLES=$(DEBUG_SMALL_SAMPLES)
    else
    CFLAGS:= $(CFLAGS) -DDEBUG_SMALL_SAMPLES=0
    endif
    ifdef SUPPRESS_USE_PRINTS
    CFLAGS:= $(CFLAGS) -DSUPPRESS_USE_PRINTS=$(SUPPRESS_USE_PRINTS)
    else
    CFLAGS:= $(CFLAGS) -DSUPPRESS_USE_PRINTS=0
    endif
    ifdef SUPPRESS_MODEL_PRINTS
    CFLAGS:= $(CFLAGS) -DSUPPRESS_MODEL_PRINTS=$(SUPPRESS_MODEL_PRINTS)
    else
    CFLAGS:= $(CFLAGS) -DSUPPRESS_MODEL_PRINTS=0
    endif
    ifdef POWERCAP_WINDOW_SIZE
    CFLAGS:= $(CFLAGS) -DPOWERCAP_WINDOW_SIZE=$(POWERCAP_WINDOW_SIZE)
    else
    CFLAGS:= $(CFLAGS) -DPOWERCAP_WINDOW_SIZE=10
    endif
    ifdef POWERCAP_OBSERVATION_PERIOD
    CFLAGS:= $(CFLAGS) -DPOWERCAP_OBSERVATION_PERIOD=$(POWERCAP_OBSERVATION_PERIOD)
    else
    CFLAGS:= $(CFLAGS) -DPOWERCAP_OBSERVATION_PERIOD=250
    endif
else
