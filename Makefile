...
#invalidation in O(1)
ifdef IPI_CONSTANT_CHILD_INVALIDATION
CFLAGS:= $(CFLAGS) -DIPI_CONSTANT_CHILD_INVALIDATION=$(IPI_CONSTANT_CHILD_INVALIDATION)
else
CFLAGS:= $(CFLAGS) -DIPI_CONSTANT_CHILD_INVALIDATION=0
endif

ifdef IPI_INTERRUPT_FUTURE
CFLAGS:= $(CFLAGS) -DIPI_INTERRUPT_FUTURE=$(IPI_INTERRUPT_FUTURE)
else
CFLAGS:= $(CFLAGS) -DIPI_INTERRUPT_FUTURE=0
endif

ifdef IPI_INTERRUPT_PAST
CFLAGS:= $(CFLAGS) -DIPI_INTERRUPT_PAST=$(IPI_INTERRUPT_PAST)
else
CFLAGS:= $(CFLAGS) -DIPI_INTERRUPT_PAST=0
endif

#calculate IPI_INTERRUPT_PAST || IPI_INTERRUPT_FUTURE
NEW_IPI_PREEMPT_COUNTER=0
ifeq ($(IPI_INTERRUPT_PAST),1)
NEW_IPI_PREEMPT_COUNTER=1
endif

ifeq ($(IPI_INTERRUPT_FUTURE),1)
NEW_IPI_PREEMPT_COUNTER=1
endif

#IPI_PREEMPT_COUNTER
CFLAGS:= $(CFLAGS) -DIPI_PREEMPT_COUNTER=$(NEW_IPI_PREEMPT_COUNTER)

ifeq ($(NEW_IPI_PREEMPT_COUNTER),1)

ifdef IPI_LONG_JMP
CFLAGS:= $(CFLAGS) -DIPI_LONG_JMP=$(IPI_LONG_JMP)
NEW_IPI_LONG_JMP=$(IPI_LONG_JMP)
else
CFLAGS:= $(CFLAGS) -DIPI_LONG_JMP=0
NEW_IPI_LONG_JMP=0
endif
...
ifdef IPI_HANDLE_INTERRUPT
CFLAGS:= $(CFLAGS) -DIPI_HANDLE_INTERRUPT=$(IPI_HANDLE_INTERRUPT)
NEW_IPI_HANDLE_INTERRUPT=$(IPI_HANDLE_INTERRUPT)
else
CFLAGS:= $(CFLAGS) -DIPI_HANDLE_INTERRUPT=0
NEW_IPI_HANDLE_INTERRUPT=0
endif

else
CFLAGS:= $(CFLAGS) -DIPI_HANDLE_INTERRUPT=0
IPI_HANDLE_INTERRUPT=0
NEW_IPI_HANDLE_INTERRUPT=0
endif
...
#IPI_POSTING
ifdef IPI_POSTING
CFLAGS:=$(CFLAGS) -DIPI_POSTING=$(IPI_POSTING)
NEW_IPI_POSTING=1
else
CFLAGS:=$(CFLAGS) -DIPI_POSTING=0
NEW_IPI_POSTING=0
endif

ifeq ($(NEW_IPI_POSTING),1)

#IPI_POSTING_STATISTICS
ifdef IPI_POSTING_STATISTICS
CFLAGS:=$(CFLAGS) -DIPI_POSTING_STATISTICS=$(IPI_POSTING_STATISTICS)
else
CFLAGS:=$(CFLAGS) -DIPI_POSTING_STATISTICS=0
endif
...
ifdef IPI_POSTING_SYNC_CHECK_FUTURE
CFLAGS:=$(CFLAGS) -DIPI_POSTING_SYNC_CHECK_FUTURE=$(IPI_POSTING_SYNC_CHECK_FUTURE)
else
CFLAGS:=$(CFLAGS) -DIPI_POSTING_SYNC_CHECK_FUTURE=0
endif

#IPI_POSTING_SYNC_CHECK_PAST
ifdef IPI_POSTING_SYNC_CHECK_PAST
CFLAGS:=$(CFLAGS) -DIPI_POSTING_SYNC_CHECK_PAST=$(IPI_POSTING_SYNC_CHECK_PAST)
else
CFLAGS:=$(CFLAGS) -DIPI_POSTING_SYNC_CHECK_PAST=0
endif

else 
#iIPI_PREEMPT_COUNTER=0
CFLAGS:=$(CFLAGS) -DIPI_POSTING_SYNC_CHECK_FUTURE=0
CFLAGS:=$(CFLAGS) -DIPI_POSTING_SYNC_CHECK_PAST=0
endif
