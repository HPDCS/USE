...
ifeq ($(PREEMPT_COUNTER),1)

ifdef INTERRUPT_FORWARD
CFLAGS:= $(CFLAGS) -DINTERRUPT_FORWARD=$(INTERRUPT_FORWARD)
else
CFLAGS:= $(CFLAGS) -DINTERRUPT_FORWARD=0
endif

ifdef INTERRUPT_SILENT
CFLAGS:= $(CFLAGS) -DINTERRUPT_SILENT=$(INTERRUPT_SILENT)
else
CFLAGS:= $(CFLAGS) -DINTERRUPT_SILENT=0
endif
...
#POSTING_SYNC_CHECK_FORWARD
ifdef POSTING_SYNC_CHECK_FORWARD
CFLAGS:=$(CFLAGS) -DPOSTING_SYNC_CHECK_FORWARD=$(POSTING_SYNC_CHECK_FORWARD)
else
CFLAGS:=$(CFLAGS) -DPOSTING_SYNC_CHECK_FORWARD=0
endif

#POSTING_SYNC_CHECK_SILENT
ifdef POSTING_SYNC_CHECK_SILENT
CFLAGS:=$(CFLAGS) -DPOSTING_SYNC_CHECK_SILENT=$(POSTING_SYNC_CHECK_SILENT)
else
CFLAGS:=$(CFLAGS) -DPOSTING_SYNC_CHECK_SILENT=0
endif
...
#POSTING==0,macros that require POSTING are setted to 0
CFLAGS:=$(CFLAGS) -DPOSTING_SYNC_CHECK_FORWARD=0
CFLAGS:=$(CFLAGS) -DPOSTING_SYNC_CHECK_SILENT=0
CFLAGS:= $(CFLAGS) -DIPI_SUPPORT=0
NEW_IPI_SUPPORT=0
...
#PHOLD_ROLL
ifdef THRESOLD_PROB_NORMAL
CFLAGS:= $(CFLAGS) -DTHRESOLD_PROB_NORMAL=$(THRESOLD_PROB_NORMAL)
endif

#PHOLD_ROLL
ifdef THRESOLD_PROB_ABNORMAL
CFLAGS:= $(CFLAGS) -DTHRESOLD_PROB_ABNORMAL=$(THRESOLD_PROB_ABNORMAL)
endif
