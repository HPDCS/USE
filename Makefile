...
#needed to PHOLD_MULTICAST
ifdef THR_POOL_SIZE
CFLAGS:= $(CFLAGS) -DTHR_POOL_SIZE=$(THR_POOL_SIZE)
else
CFLAGS:= $(CFLAGS) -DTHR_POOL_SIZE=128
endif
...
INVALIDATE_MSG_IN_EXECUTION=1
RESUMABLE_ROLLBACK=1
endif
...
ifdef INVALIDATE_MSG_IN_EXECUTION
CFLAGS:= $(CFLAGS) -DINVALIDATE_MSG_IN_EXECUTION=$(INVALIDATE_MSG_IN_EXECUTION)
NEW_INVALIDATE_MSG_IN_EXECUTION=$(INVALIDATE_MSG_IN_EXECUTION)
else
CFLAGS:= $(CFLAGS) -DINVALIDATE_MSG_IN_EXECUTION=0
NEW_INVALIDATE_MSG_IN_EXECUTION=0
endif
...
ifeq ($(NEW_SYNCH_CHECK),1)
ifdef RESUMABLE_ROLLBACK
CFLAGS:= $(CFLAGS) -DRESUMABLE_ROLLBACK=$(RESUMABLE_ROLLBACK)
NEW_RESUMABLE_ROLLBACK=$(RESUMABLE_ROLLBACK)
else
CFLAGS:= $(CFLAGS) -DRESUMABLE_ROLLBACK=0
NEW_RESUMABLE_ROLLBACK=0
endif
