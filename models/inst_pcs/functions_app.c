...
void deallocation(unsigned int me, lp_state_type *pointer, int ch, simtime_t lvt) {
	...
	if(c != NULL){
		if(c == pointer->channels){
			MODEL_WRITE(pointer->channels, c->prev);
			if(pointer->channels)
				MODEL_WRITE(pointer->channels->next, NULL);
		}
		else{
			if(c->next != NULL)
				MODEL_WRITE(c->next->prev, c->prev);
			if(c->prev != NULL)
				MODEL_WRITE(c->prev->next, c->next);
		}
...
void fading_recheck(lp_state_type *pointer) {
	...
	ch = pointer->channels;
        ...
        while(ch != NULL){
        	MODEL_WRITE(ch->sir_data->fading, Expent(1.0));
                ch = ch->prev;
        }
}
...
int allocation(lp_state_type *pointer) {
	...
	if(index != -1){
		...
		MODEL_WRITE(c->next, NULL);
		MODEL_WRITE(c->prev, pointer->channels);
		MODEL_WRITE(c->channel_id, index);
		MODEL_WRITE(c->sir_data, (sir_data_per_cell*)malloc(sizeof(sir_data_per_cell)));
		...
		if(pointer->channels != NULL)
			MODEL_WRITE(pointer->channels->next, c);

		MODEL_WRITE(pointer->channels, c);
	...
	//force this
		...
		if (summ == 0.0) {
			...
			MODEL_WRITE(c->sir_data->power, MIN_POWER);
		} else {
		  	MODEL_WRITE(c->sir_data->fading, Expent(1.0));
			MODEL_WRITE(c->sir_data->power, ((SIR_AIM * summ) / (generate_path_gain() * c->sir_data->fading)));

			if (c->sir_data->power < MIN_POWER) 
				MODEL_WRITE(c->sir_data->power, MIN_POWER);
			if (c->sir_data->power > MAX_POWER) 
				MODEL_WRITE(c->sir_data->power, MAX_POWER);
		}
