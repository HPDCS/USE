...
{
  calqueue_init();
  //_thr_pool.min_time = INFTY; ///mauro: l'ho aggiunto io per far si che all'inizio il minimo sia infinito, altrimenti rischio d tornare 0
...
void queue_insert(unsigned int receiver, simtime_t timestamp, unsigned int event_type, void *event_content, unsigned int event_size)
{
  ...
  #ifdef MYDEBUG
  printf("MYDEBUG-   queue.c/queue_insert: Inizio\n");
  #endif // MYDEBUG
  ...
  #ifdef MYDEBUG
  printf("MYDEBUG-   queue.c/queue_insert: Fine\n");
  #endif // MYDEBUG
