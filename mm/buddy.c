...
void *pool_get_memory(LID_t lid, size_t size, unsigned int numa_node) {
	long long offset,
		displacement;
	size_t fragments;
...
bool allocator_init(unsigned int objs) {
	...
	for (i = 0; i < n_prc_tot; i++) {
		...
		if(1 /*GidToKernel(gid) == kid */) {
			...
			mem_areas[lid_to_int(GidToLid(gid))] = get_segment(gid, numa_from_lp(gid));
			buddies[lid_to_int(GidToLid(gid))] = buddy_new(PER_LP_PREALLOCATED_MEMORY / BUDDY_GRANULARITY);
			continue;
		} else {
			(void)get_segment(gid, numa_from_lp(gid));
		}
	}
