...
size_t get_log_size(malloc_state *logged_state){
	...
	if (is_incremental(logged_state)) { 
		return sizeof(malloc_state) + sizeof(seed_type) + logged_state->busy_areas * sizeof(malloc_area) + logged_state->bitmap_size + logged_state->dirty_bitmap_size;
	} else {
	...
	/*if (is_incremental(logged_state)) { //tutto tranne total log size
