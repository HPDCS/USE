        ...
        (
	      ...
	      assertf(!haveLock(lp_idx), "trying to unlock without own lock %s", "\n");
            ...
            if(validity) {
                ...
                ///* LOCK TAKEN, VALID, EXTRACTED possibly not executed in current epoch of the current_lp  *///
                else if(curr_evt_state == EXTRACTED){
                    if(in_past) do_skip_inside_lock_and_goto_next(lp_idx);   // it is in past thus it has been executed
                    else        return_evt_to_main_loop();                   // it is in the future so execute it
                }
