...
do
	...
	do
		make $test NBC=1 REVERSIBLE=0 LOOKAHEAD=${lookahead} FAN_OUT=${fan_out} LOOP_COUNT=${loop_count} PERC_USED_BUCKET=${pub} ELEM_PER_BUCKET=${epb} REPORT=1 DEBUG=0 HOTSPOTS=${hs} P_HOTSPOT=${phs} CKP_PERIOD=${ck} PRINT_SCREEN=0
		mv $test ${test}_lf_hi
